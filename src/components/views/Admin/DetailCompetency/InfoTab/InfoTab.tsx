...
const InfoTab = (props: PropTypes) => {
  const { dataCompetency, onUpdate, isPendingUpdate, isSuccessUpdate } = props;
  const {
    controlUpdateInfo,
    errorsUpdateInfo,
  ...
  useEffect(() => {
    setValueUpdateInfo("title", `${dataCompetency?.title}`);
    setValueUpdateInfo("description", `${dataCompetency?.description}`);
    setValueUpdateInfo("access", Array.isArray(dataCompetency?.access) ? dataCompetency?.access : [`${dataCompetency?.access}`]);
  }, [dataCompetency]);
  ...
  return (
    <Card className="w-full p-4 lg:w-1/2">
      ...
      <CardBody>
        ...
        >
          <Skeleton isLoaded={!!dataCompetency?.title} className="rounded-lg">
            <Controller
              name="title"
          ...
          <Skeleton
            isLoaded={!!dataCompetency?.description}
          ...
          <Skeleton
            isLoaded={!!dataCompetency?.access}
            className="rounded-lg"
          >
            <Controller
              name="access"
          ...
          <Button
            ...
            type="submit"
            disabled={isPendingUpdate || !dataCompetency?._id}
          >
            {isPendingUpdate ? (
              <Spinner size="sm" />
            ) : (
