...
const InfoTab = (props: PropTypes) => {
  ...
  useEffect(() => {
    ...
    setValueUpdateInfo("file", `${data?.file}`);
  }, [data]);
  ...
  return (
    <Card className="w-full p-4 lg:w-1/2">
      <CardHeader className="flex-col items-center">
        <h1 className="w-full text-xl font-bold">SOP Information</h1>
        <p className="w-full text-small text-default-400">
          Manage information of this SOP
        </p>
      </CardHeader>
      <CardBody>
        <form
        ...
        >
          ...
          <Skeleton isLoaded={!!data?.file} className="rounded-lg">
            <Controller
              name="file"
              control={controlUpdateInfo}
              render={({ field }) => (
                <Input
                  ...
                  variant="bordered"
                  type="text"
                  isInvalid={errorsUpdateInfo.file !== undefined}
                  errorMessage={errorsUpdateInfo.file?.message}
                  className="mb-2"
                />
              )}
            />
