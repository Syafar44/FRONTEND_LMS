...
import { COLUMN_LISTS } from "./Certificate.constants";
...
const Certificate = () => {
  ...
  const renderCell = useCallback(
    (certificate: Record<string, unknown>, columnKey: any) => {
      const cellValue = certificate[columnKey as keyof typeof certificate];
      switch (columnKey) {
        case "certificate":
          return (
            <div>
              <p className="uppercase">{`${certificate.id}`}</p>
            </div>
          );
        case "competency":
          return (
            <div>
              <span>
                {typeof certificate.competency === "object" && certificate.competency !== null && "title" in certificate.competency
                  ? String((certificate.competency as { title: string }).title)
                  : ""}
              </span>
            </div>
          );
        case "actions":
          return (
            <DropdownAction
              ...
              textButton="Lihat Sertifikat"
              onPressButtonDetail={() =>
                push(`/certificate/${certificate.id}`)
              }
            />
          );
        default:
    ...
    <section>
      {Object.keys(query).length > 0 && (
        <DataTable
          ...
          data={dataCertificate?.data || []}
          emptyContent="Certificate is empty"
          isLoading={isLoadingCertificate || isRefetchingCertificate}
          renderCell={renderCell}
          totalPages={dataCertificate?.pagination?.totalPages}
        />
      )}
    </section>
  );
...
export default Certificate;
    ...
    // <div className="relative flex flex-col 2xl:flex-row gap-5">
    //   {/* Kanvas sertifikat */}
    ...
    //       {/* Code */}
    //       <div className="sm:mt-2 xl:mt-4 inline-block bg-gray-700 text-white px-3 xl:px-4 py-0.5 rounded-md xl:rounded-lg text-[7px] sm:text-xs xl:text-sm font-semibold">
    //         AGD726
    //       </div>
    ...
    //           <h2 className="text-sm sm:text-xl md:text-3xl xl:text-4xl font-bold text-primary">Syafaruddin</h2>
    //         </div>
    //         <div>
    //           <p className="text-secondary text-[7px] sm:text-[12px] md:text-sm xl:text-base">Atas kelulusannya pada kelas</p>
    //           <h3 className="text-xs sm:text-base md:text-xl xl:text-2xl font-bold text-primary">VISI & MISI</h3>
    //         </div>
    //       </div>
    ...
    //         <div>
    //           <p className="font-semibold text-[7px] sm:text-xs md:text-sm">27 Agustus 2025</p>
    ...
    //       <p>ID: <strong>AGD726</strong></p>
    //       <h3>Diselesaikan oleh: <strong>Syafaruddin</strong></h3>
    //       <p>Diadakan pada: <strong>27 Agustus 2025</strong></p>
    //       <p>Akun <strong>Syafaruddin</strong> telah terverifikasi secara resmi oleh <strong>Panglima Roqiiqu Group</strong>. Maka dari itu kami mengeluarkan sertifikat ini sebagai bukti bahwa yang bersangkutan sudah menyelesaikan kelas <strong>VISI & MISI</strong>.</p>
